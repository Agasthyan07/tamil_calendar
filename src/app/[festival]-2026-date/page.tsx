...
export async function generateStaticParams() {
    ...
    // Filter for Major Festivals or just detailed festivals we want pages for
    // For now, let's generate pages for all "Major Festival" and "Festival" types
    // Slug format: [festival-name]-2026-date

    return festivals
        .filter(f => f.type === "Major Festival" || f.type === "Festival" || f.type === "Religious")
        .map((festival) => ({
            festival: festival.name.toLowerCase().replace(/[^\w\s-]/g, '').replace(/\s+/g, '-'),
        }));
}
...
export async function generateMetadata({ params }: { params: Promise<{ festival: string }> }): Promise<Metadata> {
    ...
    if (!festivalSlug) return { title: "Festival Not Found" };

    // Reverse slug to find name (approximate) or find by slug match
    const festivals = getFestivals();
    const festival = festivals?.find(f =>
        f.name.toLowerCase().replace(/[^\w\s-]/g, '').replace(/\s+/g, '-') === festivalSlug
    );
